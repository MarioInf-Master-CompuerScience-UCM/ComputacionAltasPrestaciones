...
static void clear_data ( void )
{
	...
	#pragma omp parallel for simd schedule(guided) private(i)
	for ( i=0 ; i<size ; i++ ) {
		u[i] = 0.0f;
		v[i] = 0.0f;
		u_prev[i] = 0.0f;
...
static int allocate_data ( void )
{
	...
	u			= (float *) _mm_malloc ( 2*size*sizeof(float), align);
	v			= (float *) _mm_malloc ( 2*size*sizeof(float), align);
	u_prev		= (float *) _mm_malloc ( 2*size*sizeof(float), align);
	v_prev		= (float *) _mm_malloc ( 2*size*sizeof(float), align);
	dens		= (float *) _mm_malloc ( 2*size*sizeof(float), align);	
	dens_prev	= (float *) _mm_malloc ( 2*size*sizeof(float), align);
...
static void react ( float * d, float * u, float * v )
{
	...
	#pragma omp parallel for simd schedule(guided) private(i), reduction(max:max_velocity2) reduction(max:max_density)
	for ( i=0 ; i<size ; i++ ) {
